process checkm {
    label 'checkm'
    publishDir "${params.output}/${name}/checkm_on_unycycler/", mode: 'copy', pattern: "bins_checkm/*"
    publishDir "${params.output}/${name}/checkm_on_unycycler/", mode: 'copy', pattern: "bins_checkm_plot/*"
    input:
    set val(name), file(bins_assemblies)
    output:
    file("bins_checkm/*")
    file("bins_checkm_plot/*")
    script:
    """
    mkdir bins
