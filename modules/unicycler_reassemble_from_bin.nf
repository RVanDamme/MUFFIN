process unicycler {
    label 'unicycler'
    publishDir "${params.output}/${name}_contig_assemblies_from_bins/", mode: 'copy', pattern: "${bin_name}*.{fasta,gfa}"
    input:
    ...
    script:
    """
    unicycler -1 ${illumina[0]} -2 ${illumina[1]} -l ${ont} -o ${bin_name}_output -t ${params.cpus} --keep 0
